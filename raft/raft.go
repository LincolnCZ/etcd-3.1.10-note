...
func (r *raft) sendAppend(to uint64) {
	...
	if errt != nil || erre != nil { // send snapshot if we failed to get term or entries
		...
		m.Type = pb.MsgApp
		m.Index = pr.Next - 1 // 注意：index为啥是Next - 1；term是与Next-1对应的。但是ents是从 Next 开始的
		m.LogTerm = term
		m.Entries = ents
...
func (r *raft) becomeLeader() {
	...
	// 处理读请求时，实际上leader需要确保当前自己是不是leader、该读请求对应的commit索引是否得到了半数投票，而当一个节点刚成为leader的时候，
	//如果没有提交过任何数据，那么在它所在的这个任期（term）内的commit索引当时是并不知道的，因此在成为leader之后，需要马上提交一个no-op的
	//空日志，这样拿到该任期的第一个commit索引。
